...
public class EmpDao {
	...
	/**
	 * 传入登录名,根据登录名称查询满足条件的员工数据返回（员工编号、登录名、密码、头像数据）
	 * 登录名不存在，返回null
	 * @param empName
	 * @return
	 * @throws SQLException 
	...
	public List<Emp> queryByPage(Pager pager) throws SQLException {
		...
		while (rs.next()) {
			...
			Emp emp = new Emp(rs.getInt(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),
					rs.getDouble(8),rs.getDate(9),dept,job,rs.getString(12),rs.getString(13));
