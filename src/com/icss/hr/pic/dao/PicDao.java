...
public class PicDao {
	...
	public void insert(Pic pic) throws SQLException {
		...
		pstmt.setBinaryStream(5, pic.getPicData(),(int)pic.getPicSize());   //这个方法必须传入数据大小，必须是int型
	 ...
	 */
	 public List<Pic> query() throws SQLException {
		 ...
		 while(rs.next()) {
			 ...
			 Pic pic = new Pic(rs.getInt(1),rs.getString(2),rs.getString(3),
					 rs.getLong(4),rs.getString(5),null,rs.getTimestamp(7));
			 list.add(pic);
		 }
		 rs.close();
		 pstmt.close();
